...
set.seed(0)
path2file <-
  "/Volumes/GoogleDrive/My Drive/GEMS/BILD-AQ"
setwd(path2file)

state_input = 'NJ'

list_of_route_files <- list.files(path = 'data_NJ/route/processed', pattern = "*.geojson")

state_tract_shapefile <- st_read('shapefile/tl_2017_34_tract/tl_2017_34_tract.shp')
...
for (file in list_of_route_files){
  ...
  current_routes <- st_read(paste0('data_NJ/route/processed/', file))
  ...
  output_dir = paste0('data_NJ/route/processed/', file_name, '.csv')
