...
# Edits on Aug 20 (testing after Xiaodan's fix)
...
OD_chunks = split(subset_OD, factor(sort(rank(row.names(subset_OD))%%n)))
...
foreach(i = 1:length(OD_chunks), .packages='sf') %dopar% {
  ...
  st_write(out_route, paste0('route/processed/',state_input,'/imputed_', state_input, '_route_', i, '.geojson'), append=FALSE)
}
...
filepath_processed = paste0("route/processed/",state_input,"/")
df <- list.files(filepath_processed, pattern = "*.geojson", full.names = TRUE) %>% 
  lapply(st_read) %>% 
  bind_rows 
st_write(df, paste0('route/processed/final/',state_input,'/imputed_',state_input,'_spillover_route.geojson'), append=FALSE)
