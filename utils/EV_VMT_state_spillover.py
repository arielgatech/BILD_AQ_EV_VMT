...
#tracts_to_border_file = 'CA_tract_to_border_distance.csv'
tracts_to_border_matrix = pd.read_csv('Network/combined/CA_tract_to_border_distance.csv')
...
hb_spillover_border_fraction_3bins['dist_bin'] = hb_spillover_border_fraction_3bins['dist_bin'].replace(['bin1', 'bin2'],['bin3', 'bin3'])
hb_spillover_border_fraction_3bins = hb_spillover_border_fraction_3bins.drop(['Unnamed: 0'], axis=1)

hb_spillover_border_fraction_3bins = hb_spillover_border_fraction_3bins.groupby(['populationGroupType', 'dist_bin']).sum()['fraction'].reset_index()
...
tracts_to_border_binned['inc_bin'] = tracts_to_border_binned['populationGroupType'] + "_" + tracts_to_border_binned['dist_bin']
tracts_to_border_binned['count_inc_bin'] = tracts_to_border_binned['inc_bin'].map(tracts_to_border_binned['inc_bin'].value_counts())
...
spillover_trip_generation_state['trips_by_tract'] = spillover_trip_generation_state['TripGeneration'] * spillover_trip_generation_state['tract_frac']
...
ACS_population['total_pop']= ACS_population['low-income'] + ACS_population['medium-income'] + ACS_population['high-income']
...
print("Summary of trips distributed across CA tracts (as origin) by distance-to-border bin: ")
print(spillover_trip_generation_state.groupby('dist_bin')['trips_by_tract'].sum())

print("Total number of trips distributed across CA tracts (as origin) by distance-to-border bin: ")
...
#sample_dist_matrix['destination'] = sample_dist_matrix['destination'].astype(int)
