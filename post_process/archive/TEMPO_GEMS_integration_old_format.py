...
link_to_tempo_output = './Input/' + state_name + '/tract_output_OR_scenarios.csv'
...
align_scenarios = {'nevi': 'NEVI', 'base': 'Baseline', 'bend': 'Bookend',
                   'OR_nevi': 'NEVI', 'WA_nevi': 'NEVI', 'WA_nevi_OR_WA': 'other',
                   'OR_nevi_OR_WA': 'other'}
...
tempo_result = tempo_result.loc[tempo_result['Scenario'].isin(['NEVI', 'Baseline'])]
# drop additional scenarios existed in the data
# print(tempo_result.Scenario.unique())
...
for analysis_year in analysis_years:
    ...
    for scenario_name in scenarios:
        ...
        fleet_by_home_tract.to_csv('./Output/' + state_name + '/EV_penetration/EV_penetration_' + output_name + '_' + scenario_name + '_old_tempo.csv', index = False)
        ...
        VMT_by_thru_tract_and_veh.to_csv('./Output/'+ state_name + '/EV_VMT/raw_VMT_' + output_name + '_' + scenario_name + '_old_tempo.csv', index = False)
        ...
        VMT_by_thru_tract_and_veh['trad_VMT'] = (VMT_by_thru_tract_and_veh['HEV'] +
                                                VMT_by_thru_tract_and_veh['ICEV'] +  
    ...
    print(result.head(5))
    GEMS_result_save_name = './Output/'+ state_name + '/VMT_' + output_name + '_old_tempo.csv'
